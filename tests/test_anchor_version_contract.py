...
def test_version_migration_preserves_history_and_refuses_lossy_downgrade(isolated_postgres_engine):
    config = Config("alembic.ini")
    scripts = ScriptDirectory.from_config(config)
    assert scripts.get_revision("20260907_0011").down_revision == "20260824_0010"
    migrated_engine = isolated_postgres_engine
    with migrated_engine.begin() as connection:
        config.attributes["connection"] = connection
        command.upgrade(config, "20260907_0011")
