...
- [ ] Close Task2 review follow-ups: share temporal policy constants between persisted evidence and enforcement; add internally consistent held-out metric regression and tolerance-boundary cases that reach promotion metric branches rather than failing artifact verification first.
- [ ] Remove the legacy application-accessible destructive reset path: `/api/demo/reset` currently calls `TRUNCATE ledger_events, financing_requests RESTART IDENTITY CASCADE`, bypassing row-level immutability and deleting governed history. Keep an explicit non-mutating retirement/error response for the old endpoint, remove repository/service truncation capability, and adapt any UI/help/tests. Verify populated lifecycle, outcome/correction, deployment/job/anchor history and sealed hashes are unchanged after attempted reset. Old tests expecting erased history must become preservation tests; test-only fixture cleanup remains isolated from application paths. Do not execute the dangerous current endpoint against any database containing retained evidence.
