...
Проверяемая граница модели — воспроизведение зафиксированных артефактов и проверки окон, временных разделов и нормализации, а не гарантия отсутствия temporal leakage. Исторический выбор checkpoint TGNN использовал validation-метки, горизонт которых пересекается с ранними моментами тестового прогноза; это не строгая перспективная валидация. / 模型证据限于固定工件重放、时间窗口、划分与归一化边界检查，不保证已经消除时间泄漏。历史 TGNN 选模使用的验证标签跨度与早期测试预测时点重叠，因此不构成严格未来预测验证。

Локальная [приёмка от 2026-09-20](docs/research/original-8010-recovery-acceptance-2026-09-20.md) — исторический результат, не подтверждение доступности сервиса сегодня. Для существующей среды 8010 используйте [runbook сохранения и обслуживания](docs/runbooks/original-environment-upgrade.md); состояние CI, развёрнутая версия и текущий health проверяются отдельно. / [2026-09-20 本机验收](docs/research/original-8010-recovery-acceptance-2026-09-20.md)是历史已执行记录，不代表今天服务可用。原有 8010 环境应遵循[保留数据的维护 runbook](docs/runbooks/original-environment-upgrade.md)，分别核对 CI、实际部署版本与当前健康状态，不以历史报告替代现场检查。
