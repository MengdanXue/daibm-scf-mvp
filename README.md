...
Хост-порт `8010` выбран, чтобы не конфликтовать с другими локальными сервисами на `8000`; внутри контейнера API по-прежнему работает на `8000`. При необходимости задайте `MVP_PORT=8000` перед запуском Compose. / 默认主机端口使用 `8010`，以避开本机常见的 `8000` 端口冲突；容器内 API 仍为 `8000`。如需兼容原端口，可在启动 Compose 前设置 `MVP_PORT=8000`。
