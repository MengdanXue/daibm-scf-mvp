...
本项目属于计算机模拟场景验证，不是生产级银行系统。规范账本仍是 PostgreSQL 关联哈希链；可选的本地 Hyperledger Fabric 仅通过 Gateway/outbox 锚定哈希。这不是生产级区块链，也不证明生产级共识、性能或容错能力。系统已实现最小 GCN–BiLSTM–MLP TGNN 与 ONNX 推理，透明规则模型仍作为独立工程基线。这里的“闭环”指“申请→评估→决策→控制→审计”，不包含实际违约结果回流和自动再训练。交易字段指纹只能证明已录入字段的一致性，不替代原始文件或外部税票平台核验。MVP 不声称复现完整 TGNN 架构或原论文指标，也不声称实现 PoA+、ZKP 或银行系统集成。
